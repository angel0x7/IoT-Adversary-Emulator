...
class BotManager:
    ...
    def on_message_callback(self, client, userdata, msg):
        try:
            # Nettoyage des bots morts (threads arrêtés) avant de spawner
            dead_bots = [bid for bid, bot in self.running_bots.items() if not bot.is_running]
            for bid in dead_bots:
                del self.running_bots[bid]

            data = json.loads(msg.payload.decode())
            if data.get("action") == "SPAWN":
                specific_name = data.get("name")
                if specific_name:
                    self.spawn_bot(specific_name)
                else:
                    for _ in range(data.get("count", 1)):
                        # ID unique basé sur l'heure pour éviter les collisions
                        new_id = f"mkr_extra_{int(time.time() * 1000) % 10000}"
                        self.spawn_bot(new_id)
        except Exception as e:
            print(f"[!] Erreur Manager Callback : {e}")
